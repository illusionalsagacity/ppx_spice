      ...
      let rhs_list =
        ...
        :: List.mapi
             (fun i arg ->
               ...
               Exp.apply ~attrs:[ Utils.attr_uapp ] ~loc:pcd_loc
                 (Option.get encoder)
...
(* O(n) nested matching for variant arg decoding - replaces O(n²) pattern generation *)
let generate_arg_decoder generator_settings args constructor_name =
  ...
  let generate_decode_expr i (_, decoder) =
    ...
    in
    [%expr [%e Option.get decoder] (Belt.Array.getExn json_arr [%e idx])]
  in
...
let generate_codecs ({ do_encode; do_decode } as generator_settings)
  ...
  let parsed_decls, count_has_attr =
    List.fold_left
      (fun (acc, count) decl ->
        let parsed = parse_decl generator_settings decl in
        (parsed :: acc, if parsed.has_attr_as then count + 1 else count))
      ([], 0) constr_decls
  in
  let parsed_decls = List.rev parsed_decls in
  let has_attr_as =
